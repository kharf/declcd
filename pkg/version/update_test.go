...
func runUpdateTestCase(t *testing.T, ctx context.Context, tc updateTestCase) {
	...
	patchedAvailableUpdate := patchFile(tc.haveAvailableUpdate, projectDir)

	update, err := updater.Update(ctx, patchedAvailableUpdate)
	if tc.wantErr != "" {
	...
	switch target := patchedAvailableUpdate.Target.(type) {
	case *version.ContainerUpdateTarget:
		split := strings.Split(target.Image, ":")
		if patchedAvailableUpdate.Integration == version.PR {
			assert.Equal(t, target.GetStructValue(), fmt.Sprintf("%s:%s", split[0], patchedAvailableUpdate.CurrentVersion))
		} else {
			assert.Equal(t, target.GetStructValue(), fmt.Sprintf("%s:%s", split[0], patchedAvailableUpdate.NewVersion))
		}
	case *version.ChartUpdateTarget:
		if patchedAvailableUpdate.Integration == version.PR {
			assert.Equal(t, target.GetStructValue(), patchedAvailableUpdate.CurrentVersion)
		} else {
			assert.Equal(t, target.GetStructValue(), patchedAvailableUpdate.NewVersion)
		}
	}

	haveFile, err := os.Open(patchedAvailableUpdate.File)
	assert.NilError(t, err)
