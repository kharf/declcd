...
func (e *ExtendedDynamicClient) Apply(
	...
	if err := e.dynamicClient.Apply(ctx, obj.Unstructured, fieldManager, opts...); err != nil {
		...
		if ok && statusErr.Status().Reason == v1.StatusReasonConflict &&
			...
			for _, cause := range causes {
				if err := deleteIgnoredFields(cause.Field, unstrObj, obj.Metadata); err != nil {
					return err
				}
			}

			// Retry without ignored fields
			if err := e.dynamicClient.Apply(ctx, obj.Unstructured, fieldManager, opts...); err != nil {
				return err
			}
		} else {
